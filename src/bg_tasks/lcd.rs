...
pub async fn display_stats_on_lcd(
    ...
    let mut display = match setup_i2c(&settings) {
        Ok(d) => {
            logging::log!(
                "[LCD] LCD device configured successfully: I2C device /dev/i2c-{cur_lcd_device}, address 0x{cur_lcd_addr}."
            );
            d
        }
        Err(err) => {
            logging::error!("[ERROR][LCD] LCD device configuration failed: {err}");
            return;
        }
    };
        ...
        select! {
            settings = bg_tasks_cmds_rx.recv() => {
                if let Ok(BgTasksCmds::ApplySettings(s)) = settings
                    ...
                    {
                        logging::log!("[LCD] Disabling LCD device on /dev/i2c-{cur_lcd_device}, address 0x{cur_lcd_addr}");
            ...
            _ = update_lcd.tick() => {
                ...
                logging::log!("[LCD] Updating statistics on LCD device");
...
impl Hardware for Pcf8574 {
    ...
    fn apply(&mut self) {
        if let Err(err) = self.dev.smbus_write_byte(self.data) {
            logging::error!("[ERROR][LCD] LCD I2C communication error: {err}");
        }
    }
}
