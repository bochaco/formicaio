...
1. NEVER invent, guess, or fabricate node IDs, names, counts, balances, or any other data. \
All node information MUST come from a tool call — prior conversation history may be stale. \
If you have not yet called a tool for the current question, call it now.\n\
2. Report tool results accurately. Do not add, remove, or modify data that was not in the tool response.\n\
3. If a tool returns an error, tell the user exactly what the error was. Do not guess the answer.\n\
4. An empty tool result (zero nodes, empty list, node_count 0) is VALID DATA. \
It means Formicaio genuinely has no nodes. Tell the user there are no nodes. \
NEVER invent nodes to fill an empty result.\n\
5. Be concise and factual. Format numbers clearly. Do not pad responses with speculative commentary.\n\
6. When asked to PERFORM AN ACTION (start, stop, restart, recycle, upgrade, delete nodes), \
you MUST actually call the tool — do NOT just describe what you would do or explain the steps. \
Call the tool NOW.\n\
7. NEVER use placeholder text as a tool argument. Every argument passed to a tool MUST be a \
...
8. For BATCH OPERATIONS (e.g. 'restart all stopped nodes', 'start all inactive nodes'): \
...
Make ONE action tool call per turn, wait for its result, then proceed to the next node. \
Do NOT batch or combine action calls — call them one at a time.";

// Additional instructions appended to the system prompt when running an autonomous cycle
const AUTONOMOUS_MODE_PROMPT: &str = "Operational context: autonomous health monitoring.\n\
- Call nodes_instances to assess current node states.\n\
- Start any nodes that are stopped or inactive.\n\
- Take NO action if all nodes are healthy — do not call action tools needlessly.\n\
- Be minimal: only take actions that are directly needed.\n\
- After all tool calls, respond with a short plain-text summary of what you found and what \
you did. If nothing was needed, respond with 'All nodes healthy — no action taken'.";

// User message sent to trigger the autonomous cycle
const HEALTH_CHECK_PROMPT: &str = "Perform a health check on all nodes now. Start any that are stopped or inactive, then report your findings.";
...
pub async fn process_chat_turn(
    ...
    let mut messages: Vec<LlmMessage> = vec![LlmMessage::system(build_system_prompt(
        ...
        tokio::select! {
            _ = check_interval.tick() => {
                ...
                if enabled {
                    ...
                    check_interval.reset_after(check_interval.period());
                }
            }
            cmd = cmds_rx.recv() => {
                match cmd {
...
async fn run_monitoring_cycle(app_ctx: &AppContext, node_manager: &NodeManager) {
    ...
    let messages = vec![
        LlmMessage::system(build_system_prompt(AUTONOMOUS_MODE_PROMPT)),
        LlmMessage::user(HEALTH_CHECK_PROMPT),
    ];
        ...
        while let Some(event) = stream.next().await {
            match event {
                ...
                Ok(StreamEvent::ToolCallDelta {
                    ...
                    if let Some((_, _, pname, pargs)) = pending.iter_mut().find(|p| p.0 == index) {
                        pargs.push_str(&arguments_delta);
                        if let Some(n) = name {
                            *pname = n;
                        }
                    } else {
