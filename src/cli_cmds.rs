...
#[cfg(feature = "ssr")]
use crate::types::{IpVersion, NodesInstancesInfo};
...
pub struct NodeOptsCmd {
    /// IP version for the node to use: ipv4, ipv6, or dual (default: dual).
    #[structopt(long, default_value = "dual")]
    ip_version: String,
        ...
        let res = match &self {
            ...
            CliCommands::Nodes(NodesSubcommands::Create(node_opts_cmd)) => {
                let node_opts = NodeOpts {
                    ip_version: node_opts_cmd.ip_version.parse().unwrap_or_default(),
        ...
        match &self {
            ...
            CliCommands::Nodes(NodesSubcommands::Create(opts)) => {
                ...
                if opts.count > 1 {
                    ...
                    // TODO: use some crate which performs this serialisation
                    let body = format!(
                        "node_opts[ip_version]={}&node_opts[port]={}&node_opts[metrics_port]={}&node_opts[rewards_addr]={}&node_opts[node_logs]={}&node_opts[auto_start]={}&node_opts[data_dir_path]={}",
                        ip_version.as_str(),
                        opts.port,
                        opts.metrics_port,
                        opts.rewards_addr,
            ...
            CliCmdResponse::Nodes(nodes, extended) => {
                if *extended {
                    for info in nodes {
                        ...
                        table.add_row(row!["IP Version", info.ip_version.as_str()]);
                        table.add_row(row![
                            "Data dir",
                            value_or_dash(
