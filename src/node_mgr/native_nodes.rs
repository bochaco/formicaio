...
#[cfg(windows)]
const NODE_BIN_NAME: &str = "saorsa-node.exe";
#[cfg(not(windows))]
const NODE_BIN_NAME: &str = "saorsa-node";
...
const NODE_LOG_FILENAME_PREFIX: &str = "saorsa-node.";

// Consts used to download node binary from GitHub releases
const SAORSA_NODE_GITHUB_REPO: &str = "WithAutonomi/ant-node";
...
// Determine the platform-specific archive name for downloading saorsa-node
fn get_platform_archive_name() -> Result<String, NativeNodesError> {
    let arch = std::env::consts::ARCH;
    let os = std::env::consts::OS;

    let name = match (os, arch) {
        ("linux", "x86_64") => "saorsa-node-cli-linux-x64.tar.gz",
        ("linux", "aarch64") => "saorsa-node-cli-linux-arm64.tar.gz",
        ("macos", "x86_64") => "saorsa-node-cli-macos-x64.tar.gz",
        ("macos", "aarch64") => "saorsa-node-cli-macos-arm64.tar.gz",
        ("windows", "x86_64") => "saorsa-node-cli-windows-x64.zip",
...
impl NativeNodes {
    ...
    pub async fn spawn_new_node(
        ...
        let mut args = vec![
            "--port".to_string(),
            port.to_string(),
            "--ip-version".to_string(),
            node_info.ip_version.as_str().to_string(),
    ...
    async fn get_node_version_and_peer_id(
        ...
        let only_ipv4 = matches!(node_info.ip_version, crate::types::IpVersion::Ipv4);
    ...
    pub async fn upgrade_master_node_binary(
        ...
        let version_to_download = match version {
            ...
            None => {
                let url =
                    format!("{GITHUB_API_URL}/repos/{SAORSA_NODE_GITHUB_REPO}/releases/latest");
        ...
        let download_url = format!(
            "https://github.com/{SAORSA_NODE_GITHUB_REPO}/releases/download/v{version_to_download}/{archive_name}"
        );
    ...
    // Log files are named saorsa-node.YYYY-MM-DD.log and rotate daily.
